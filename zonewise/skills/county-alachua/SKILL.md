...
name: county-alachua
description: >
  Zoning intelligence for Alachua County, FL.
  8 jurisdictions, FDOR co_no 01.
  Portal: municode. Anti-scrape: false.
  Triggers: Alachua, alachua county, Gainesville, co_no 01.
co_no: 01
county_slug: alachua
...
# Alachua County — Zoning Intelligence

**Seat**: Gainesville | **Pop**: 282,840 | **Municipalities**: 8
**FDOR co_no**: 01 | **Portal**: MUNICODE | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Alachua%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.1&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.1&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Alachua County Florida zoning map"`
2. `"Alachua County Florida municode zoning ordinance"`
3. `"Alachua County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/alachua_county
...
**Container**: `modal-county-alachua`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='alachua'` → Traycer auto-issue.
...
| Municipalities | 8 |
| Portal | MUNICODE |
| Municode | [alachua_county](https://library.municode.com/fl/alachua_county) |
| Anti-scrape | No |
| Rate limit | 60 rpm |
| Phase | P3 |
...
Trigger phrases: "Alachua", "alachua county", "Gainesville", "co_no 01"
