...
name: county-baker
description: >
  Zoning intelligence for Baker County, FL.
  2 jurisdictions, FDOR co_no 02.
  Portal: municode. Anti-scrape: false.
  Triggers: Baker, baker county, Macclenny, co_no 02.
co_no: 02
county_slug: baker
...
# Baker County — Zoning Intelligence

**Seat**: Macclenny | **Pop**: 29,210 | **Municipalities**: 2
**FDOR co_no**: 02 | **Portal**: MUNICODE | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Baker%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.2&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.2&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Baker County Florida zoning map"`
2. `"Baker County Florida municode zoning ordinance"`
3. `"Baker County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/baker_county
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='baker'` → Traycer auto-issue.
...
| Municipalities | 2 |
| Portal | MUNICODE |
| Municode | [baker_county](https://library.municode.com/fl/baker_county) |
| Anti-scrape | No |
| Rate limit | 30 rpm |
| Phase | P3 |
...
Trigger phrases: "Baker", "baker county", "Macclenny", "co_no 02"
