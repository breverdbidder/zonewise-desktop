...
name: county-baker
description: >
  Zoning intelligence for Baker County, FL (FDOR co_no: 2).
  ...
  overlay districts. Triggers on: Baker County, co_no 2,
  any address in Baker County Florida.
supabase_county_filter: "county=ilike.%25Baker%25"
co_no: 2
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 2 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Baker County Florida zoning ordinance municode",
    "Baker County Florida GIS zoning layer ArcGIS",
    "Baker County Florida online zoning map portal",
    "site:municode.com Baker county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'baker'})
```
...
- Fallback: FDOR fl_parcels table (co_no=2)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-baker`
