...
name: county-bay
description: >
  Zoning intelligence for Bay County, FL (FDOR co_no: 3).
  ...
  overlay districts. Triggers on: Bay County, co_no 3,
  any address in Bay County Florida.
supabase_county_filter: "county=ilike.%25Bay%25"
co_no: 3
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 3 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Bay County Florida zoning ordinance municode",
    "Bay County Florida GIS zoning layer ArcGIS",
    "Bay County Florida online zoning map portal",
    "site:municode.com Bay county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'bay'})
```
...
- Fallback: FDOR fl_parcels table (co_no=3)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-bay`
