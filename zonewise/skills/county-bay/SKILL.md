...
name: county-bay
description: >
  Zoning intelligence for Bay County, FL.
  8 jurisdictions, FDOR co_no 03.
  Portal: municode. Anti-scrape: false.
  Triggers: Bay, bay county, Panama City, co_no 03.
co_no: 03
county_slug: bay
...
# Bay County — Zoning Intelligence

**Seat**: Panama City | **Pop**: 180,076 | **Municipalities**: 8
**FDOR co_no**: 03 | **Portal**: MUNICODE | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Bay%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.3&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.3&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Bay County Florida zoning map"`
2. `"Bay County Florida municode zoning ordinance"`
3. `"Bay County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/bay_county
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='bay'` → Traycer auto-issue.
...
| Municipalities | 8 |
| Portal | MUNICODE |
| Municode | [bay_county](https://library.municode.com/fl/bay_county) |
| Anti-scrape | No |
| Rate limit | 60 rpm |
| Phase | P3 |
...
Trigger phrases: "Bay", "bay county", "Panama City", "co_no 03"
