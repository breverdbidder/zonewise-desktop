...
description: >
  Zoning intelligence for Bradford County, FL (FDOR co_no: 4).
  ...
  overlay districts. Triggers on: Bradford County, co_no 4,
  any address in Bradford County Florida.
supabase_county_filter: "county=ilike.%25Bradford%25"
co_no: 4
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 4 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Bradford County Florida zoning ordinance municode",
    "Bradford County Florida GIS zoning layer ArcGIS",
    "Bradford County Florida online zoning map portal",
    "site:municode.com Bradford county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'bradford'})
```
...
- Fallback: FDOR fl_parcels table (co_no=4)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-bradford`
