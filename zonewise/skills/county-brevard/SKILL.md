...
name: county-brevard
description: >
  Zoning intelligence for Brevard County, FL.
  17 jurisdictions, FDOR co_no 05.
  Portal: arcgis. Anti-scrape: false.
  Triggers: Brevard, brevard county, Titusville, co_no 05.
co_no: 05
county_slug: brevard
portal_type: arcgis
anti_scrape: false
...
**Seat**: Titusville | **Pop**: 617,176 | **Municipalities**: 17
**FDOR co_no**: 05 | **Portal**: ARCGIS | **Phase**: P0
...
```
GET /jurisdictions?county=ilike.%25Brevard%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.5&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.5&parcel_id=eq.{parcel_id}&select=*
```

### GIS Direct Query (ArcGIS)
```
GET https://gis.brevardfl.gov/gissrv/rest/services/Planning_Development/Zoning_WKID2881/MapServer/0/query?where=1=1&geometry={lng},{lat}&geometryType=esriGeometryPoint&spatialRel=esriSpatialRelIntersects&outFields=ZONING&f=json
```
Zone field: `ZONING`
...
Queries:
1. `"Brevard County Florida zoning map"`
2. `"Brevard County Florida municode zoning ordinance"`
3. `"Brevard County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/melbourne
...
**Container**: `modal-county-brevard`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='brevard'` → Traycer auto-issue.
...
| Municipalities | 17 |
| Portal | ARCGIS |
| Municode | [melbourne](https://library.municode.com/fl/melbourne) |
| Anti-scrape | No |
| Rate limit | 60 rpm |
| Phase | P0 |
...
Trigger phrases: "Brevard", "brevard county", "Titusville", "co_no 05"
