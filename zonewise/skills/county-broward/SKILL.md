...
name: county-broward
description: >
  Zoning intelligence for Broward County, FL.
  31 jurisdictions, FDOR co_no 06.
  Portal: arcgis. Anti-scrape: true.
  Triggers: Broward, broward county, Fort Lauderdale, co_no 06.
co_no: 06
county_slug: broward
portal_type: arcgis
anti_scrape: true
...
# Broward County — Zoning Intelligence

**Seat**: Fort Lauderdale | **Pop**: 1,952,778 | **Municipalities**: 31
**FDOR co_no**: 06 | **Portal**: ARCGIS | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Broward%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.6&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.6&parcel_id=eq.{parcel_id}&select=*
```

### GIS Direct Query (ArcGIS)
```
GET https://gis.broward.org/arcgis/rest/services/Zoning/MapServer/query?where=1=1&geometry={lng},{lat}&geometryType=esriGeometryPoint&spatialRel=esriSpatialRelIntersects&outFields=ZONE_CODE&f=json
```
...
Queries:
1. `"Broward County Florida zoning map"`
2. `"Broward County Florida municode zoning ordinance"`
3. `"Broward County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/broward_county
...
**Container**: `modal-county-broward`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='broward'` → Traycer auto-issue.
...
| Municipalities | 31 |
| Portal | ARCGIS |
| Municode | [broward_county](https://library.municode.com/fl/broward_county) |
| Anti-scrape | ⚠️ YES |
| Rate limit | 30 rpm |
| Phase | P3 |
...
Trigger phrases: "Broward", "broward county", "Fort Lauderdale", "co_no 06"
