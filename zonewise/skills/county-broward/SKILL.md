...
name: county-broward
description: >
  Zoning intelligence for Broward County, FL (FDOR co_no: 6).
  ...
  overlay districts. Triggers on: Broward County, co_no 6,
  any address in Broward County Florida.
supabase_county_filter: "county=ilike.%25Broward%25"
co_no: 6
portal_type: unknown
anti_scrape: true
rate_limit_rpm: 10
...
> **co_no**: 6 | **Phase**: P3 | **Anti-scrape**: true | **Rate limit**: 10 rpm
...
```python
queries = [
    "Broward County Florida zoning ordinance municode",
    "Broward County Florida GIS zoning layer ArcGIS",
    "Broward County Florida online zoning map portal",
    "site:municode.com Broward county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'broward'})
```
...
- Fallback: FDOR fl_parcels table (co_no=6)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-broward`
