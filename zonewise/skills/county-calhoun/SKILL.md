...
name: county-calhoun
description: >
  Zoning intelligence for Calhoun County, FL.
  3 jurisdictions, FDOR co_no 07.
  Portal: municode. Anti-scrape: false.
  Triggers: Calhoun, calhoun county, Blountstown, co_no 07.
co_no: 07
county_slug: calhoun
...
# Calhoun County — Zoning Intelligence

**Seat**: Blountstown | **Pop**: 14,444 | **Municipalities**: 3
**FDOR co_no**: 07 | **Portal**: MUNICODE | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Calhoun%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.7&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.7&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Calhoun County Florida zoning map"`
2. `"Calhoun County Florida municode zoning ordinance"`
3. `"Calhoun County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/calhoun_county
...
**Container**: `modal-county-calhoun`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='calhoun'` → Traycer auto-issue.
...
| Municipalities | 3 |
| Portal | MUNICODE |
| Municode | [calhoun_county](https://library.municode.com/fl/calhoun_county) |
| Anti-scrape | No |
| Rate limit | 20 rpm |
| Phase | P3 |
...
Trigger phrases: "Calhoun", "calhoun county", "Blountstown", "co_no 07"
