---
name: county-charlotte
description: >
  Zoning intelligence for Charlotte County, FL.
  2 jurisdictions, FDOR co_no 08.
  Portal: arcgis. Anti-scrape: false.
  Triggers: Charlotte, charlotte county, Punta Gorda, co_no 08.
co_no: 08
...
# Charlotte County — Zoning Intelligence

**Seat**: Punta Gorda | **Pop**: 188,910 | **Municipalities**: 2
**FDOR co_no**: 08 | **Portal**: ARCGIS | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Charlotte%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.8&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.8&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Charlotte County Florida zoning map"`
2. `"Charlotte County Florida municode zoning ordinance"`
3. `"Charlotte County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/charlotte_county
...
**Container**: `modal-county-charlotte`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='charlotte'` → Traycer auto-issue.
...
| Municipalities | 2 |
| Portal | ARCGIS |
| Municode | [charlotte_county](https://library.municode.com/fl/charlotte_county) |
| Anti-scrape | No |
| Rate limit | 60 rpm |
| Phase | P3 |
...
Trigger phrases: "Charlotte", "charlotte county", "Punta Gorda", "co_no 08"
