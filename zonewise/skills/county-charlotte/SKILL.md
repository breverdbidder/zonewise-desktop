...
description: >
  Zoning intelligence for Charlotte County, FL (FDOR co_no: 8).
  ...
  overlay districts. Triggers on: Charlotte County, co_no 8,
  any address in Charlotte County Florida.
supabase_county_filter: "county=ilike.%25Charlotte%25"
co_no: 8
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 8 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Charlotte County Florida zoning ordinance municode",
    "Charlotte County Florida GIS zoning layer ArcGIS",
    "Charlotte County Florida online zoning map portal",
    "site:municode.com Charlotte county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'charlotte'})
```
...
- Fallback: FDOR fl_parcels table (co_no=8)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-charlotte`
