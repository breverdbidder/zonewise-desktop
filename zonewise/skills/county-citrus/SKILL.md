...
name: county-citrus
description: >
  Zoning intelligence for Citrus County, FL.
  4 jurisdictions, FDOR co_no 09.
  Portal: municode. Anti-scrape: false.
  Triggers: Citrus, citrus county, Inverness, co_no 09.
co_no: 09
county_slug: citrus
...
# Citrus County — Zoning Intelligence

**Seat**: Inverness | **Pop**: 152,434 | **Municipalities**: 4
**FDOR co_no**: 09 | **Portal**: MUNICODE | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Citrus%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.9&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.9&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Citrus County Florida zoning map"`
2. `"Citrus County Florida municode zoning ordinance"`
3. `"Citrus County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/citrus_county
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='citrus'` → Traycer auto-issue.
...
| Municipalities | 4 |
| Portal | MUNICODE |
| Municode | [citrus_county](https://library.municode.com/fl/citrus_county) |
| Anti-scrape | No |
| Rate limit | 40 rpm |
| Phase | P3 |
...
Trigger phrases: "Citrus", "citrus county", "Inverness", "co_no 09"
