...
name: county-collier
description: >
  Zoning intelligence for Collier County, FL.
  3 jurisdictions, FDOR co_no 11.
  Portal: arcgis. Anti-scrape: true.
  Triggers: Collier, collier county, Naples, co_no 11.
co_no: 11
county_slug: collier
portal_type: arcgis
anti_scrape: true
...
# Collier County — Zoning Intelligence

**Seat**: Naples | **Pop**: 384,902 | **Municipalities**: 3
**FDOR co_no**: 11 | **Portal**: ARCGIS | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Collier%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.11&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.11&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Collier County Florida zoning map"`
2. `"Collier County Florida municode zoning ordinance"`
3. `"Collier County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/collier_county
...
**Container**: `modal-county-collier`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='collier'` → Traycer auto-issue.
...
| Municipalities | 3 |
| Portal | ARCGIS |
| Municode | [collier_county](https://library.municode.com/fl/collier_county) |
| Anti-scrape | ⚠️ YES |
| Rate limit | 30 rpm |
| Phase | P3 |
...
Trigger phrases: "Collier", "collier county", "Naples", "co_no 11"
