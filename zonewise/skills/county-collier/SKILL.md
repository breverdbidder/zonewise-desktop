...
name: county-collier
description: >
  Zoning intelligence for Collier County, FL (FDOR co_no: 11).
  ...
  overlay districts. Triggers on: Collier County, co_no 11,
  any address in Collier County Florida.
supabase_county_filter: "county=ilike.%25Collier%25"
co_no: 11
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 11 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Collier County Florida zoning ordinance municode",
    "Collier County Florida GIS zoning layer ArcGIS",
    "Collier County Florida online zoning map portal",
    "site:municode.com Collier county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'collier'})
```
...
- Fallback: FDOR fl_parcels table (co_no=11)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-collier`
