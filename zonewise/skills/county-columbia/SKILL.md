---
name: county-columbia
description: >
  Zoning intelligence for Columbia County, FL.
  4 jurisdictions, FDOR co_no 12.
  Portal: municode. Anti-scrape: false.
  Triggers: Columbia, columbia county, Lake City, co_no 12.
co_no: 12
...
# Columbia County — Zoning Intelligence

**Seat**: Lake City | **Pop**: 71,513 | **Municipalities**: 4
**FDOR co_no**: 12 | **Portal**: MUNICODE | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Columbia%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.12&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.12&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Columbia County Florida zoning map"`
2. `"Columbia County Florida municode zoning ordinance"`
3. `"Columbia County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/columbia_county
...
**Container**: `modal-county-columbia`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='columbia'` → Traycer auto-issue.
...
| Municipalities | 4 |
| Portal | MUNICODE |
| Municode | [columbia_county](https://library.municode.com/fl/columbia_county) |
| Anti-scrape | No |
| Rate limit | 40 rpm |
| Phase | P3 |
...
Trigger phrases: "Columbia", "columbia county", "Lake City", "co_no 12"
