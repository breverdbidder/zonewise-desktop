...
description: >
  Zoning intelligence for Columbia County, FL (FDOR co_no: 12).
  ...
  overlay districts. Triggers on: Columbia County, co_no 12,
  any address in Columbia County Florida.
supabase_county_filter: "county=ilike.%25Columbia%25"
co_no: 12
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 12 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Columbia County Florida zoning ordinance municode",
    "Columbia County Florida GIS zoning layer ArcGIS",
    "Columbia County Florida online zoning map portal",
    "site:municode.com Columbia county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'columbia'})
```
...
- Fallback: FDOR fl_parcels table (co_no=12)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-columbia`
