...
name: county-desoto
description: >
  Zoning intelligence for DeSoto County, FL.
  1 jurisdictions, FDOR co_no 14.
  Portal: municode. Anti-scrape: false.
  Triggers: DeSoto, desoto county, Arcadia, co_no 14.
co_no: 14
county_slug: desoto
...
# DeSoto County — Zoning Intelligence

**Seat**: Arcadia | **Pop**: 35,865 | **Municipalities**: 1
**FDOR co_no**: 14 | **Portal**: MUNICODE | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25DeSoto%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.14&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.14&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"DeSoto County Florida zoning map"`
2. `"DeSoto County Florida municode zoning ordinance"`
3. `"DeSoto County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/desoto_county
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='desoto'` → Traycer auto-issue.
...
| Municipalities | 1 |
| Portal | MUNICODE |
| Municode | [desoto_county](https://library.municode.com/fl/desoto_county) |
| Anti-scrape | No |
| Rate limit | 20 rpm |
| Phase | P3 |
...
Trigger phrases: "DeSoto", "desoto county", "Arcadia", "co_no 14"
