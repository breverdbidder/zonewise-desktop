...
name: county-desoto
description: >
  Zoning intelligence for DeSoto County, FL (FDOR co_no: 14).
  ...
  overlay districts. Triggers on: DeSoto County, co_no 14,
  any address in DeSoto County Florida.
supabase_county_filter: "county=ilike.%25DeSoto%25"
co_no: 14
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 14 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "DeSoto County Florida zoning ordinance municode",
    "DeSoto County Florida GIS zoning layer ArcGIS",
    "DeSoto County Florida online zoning map portal",
    "site:municode.com DeSoto county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'desoto'})
```
...
- Fallback: FDOR fl_parcels table (co_no=14)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-desoto`
