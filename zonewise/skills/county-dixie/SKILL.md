...
name: county-dixie
description: >
  Zoning intelligence for Dixie County, FL (FDOR co_no: 15).
  ...
  overlay districts. Triggers on: Dixie County, co_no 15,
  any address in Dixie County Florida.
supabase_county_filter: "county=ilike.%25Dixie%25"
co_no: 15
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 15 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Dixie County Florida zoning ordinance municode",
    "Dixie County Florida GIS zoning layer ArcGIS",
    "Dixie County Florida online zoning map portal",
    "site:municode.com Dixie county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'dixie'})
```
...
- Fallback: FDOR fl_parcels table (co_no=15)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-dixie`
