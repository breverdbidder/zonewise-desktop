...
name: county-dixie
description: >
  Zoning intelligence for Dixie County, FL.
  3 jurisdictions, FDOR co_no 15.
  Portal: municode. Anti-scrape: false.
  Triggers: Dixie, dixie county, Cross City, co_no 15.
co_no: 15
county_slug: dixie
...
# Dixie County — Zoning Intelligence

**Seat**: Cross City | **Pop**: 16,742 | **Municipalities**: 3
**FDOR co_no**: 15 | **Portal**: MUNICODE | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Dixie%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.15&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.15&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Dixie County Florida zoning map"`
2. `"Dixie County Florida municode zoning ordinance"`
3. `"Dixie County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/dixie_county
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='dixie'` → Traycer auto-issue.
...
| Municipalities | 3 |
| Portal | MUNICODE |
| Municode | [dixie_county](https://library.municode.com/fl/dixie_county) |
| Anti-scrape | No |
| Rate limit | 20 rpm |
| Phase | P3 |
...
Trigger phrases: "Dixie", "dixie county", "Cross City", "co_no 15"
