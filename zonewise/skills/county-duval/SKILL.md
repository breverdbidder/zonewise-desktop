...
name: county-duval
description: >
  Zoning intelligence for Duval County, FL.
  1 jurisdictions, FDOR co_no 16.
  Portal: arcgis. Anti-scrape: true.
  Triggers: Duval, duval county, Jacksonville, co_no 16.
co_no: 16
county_slug: duval
portal_type: arcgis
anti_scrape: true
...
# Duval County — Zoning Intelligence

**Seat**: Jacksonville | **Pop**: 979,567 | **Municipalities**: 1
**FDOR co_no**: 16 | **Portal**: ARCGIS | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Duval%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.16&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.16&parcel_id=eq.{parcel_id}&select=*
```

### GIS Direct Query (ArcGIS)
```
GET https://duvalfl.maps.arcgis.com/arcgis/rest/services/Zoning/MapServer/query?where=1=1&geometry={lng},{lat}&geometryType=esriGeometryPoint&spatialRel=esriSpatialRelIntersects&outFields=ZONE_DIST&f=json
```
Zone field: `ZONE_DIST`
...
Queries:
1. `"Duval County Florida zoning map"`
2. `"Duval County Florida municode zoning ordinance"`
3. `"Duval County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/jacksonville
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='duval'` → Traycer auto-issue.
...
| Municipalities | 1 |
| Portal | ARCGIS |
| Municode | [jacksonville](https://library.municode.com/fl/jacksonville) |
| Anti-scrape | ⚠️ YES |
| Rate limit | 30 rpm |
| Phase | P3 |
...
Trigger phrases: "Duval", "duval county", "Jacksonville", "co_no 16"
