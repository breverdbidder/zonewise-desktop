...
name: county-duval
description: >
  Zoning intelligence for Duval County, FL (FDOR co_no: 16).
  ...
  overlay districts. Triggers on: Duval County, co_no 16,
  any address in Duval County Florida.
supabase_county_filter: "county=ilike.%25Duval%25"
co_no: 16
portal_type: unknown
anti_scrape: true
rate_limit_rpm: 10
...
> **co_no**: 16 | **Phase**: P3 | **Anti-scrape**: true | **Rate limit**: 10 rpm
...
```python
queries = [
    "Duval County Florida zoning ordinance municode",
    "Duval County Florida GIS zoning layer ArcGIS",
    "Duval County Florida online zoning map portal",
    "site:municode.com Duval county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'duval'})
```
...
- Fallback: FDOR fl_parcels table (co_no=16)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-duval`
