---
name: county-escambia
description: >
  Zoning intelligence for Escambia County, FL.
  2 jurisdictions, FDOR co_no 17.
  Portal: municode. Anti-scrape: false.
  Triggers: Escambia, escambia county, Pensacola, co_no 17.
co_no: 17
...
# Escambia County — Zoning Intelligence

**Seat**: Pensacola | **Pop**: 317,887 | **Municipalities**: 2
**FDOR co_no**: 17 | **Portal**: MUNICODE | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Escambia%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.17&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.17&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Escambia County Florida zoning map"`
2. `"Escambia County Florida municode zoning ordinance"`
3. `"Escambia County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/escambia_county
...
**Container**: `modal-county-escambia`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='escambia'` → Traycer auto-issue.
...
| Municipalities | 2 |
| Portal | MUNICODE |
| Municode | [escambia_county](https://library.municode.com/fl/escambia_county) |
| Anti-scrape | No |
| Rate limit | 60 rpm |
| Phase | P3 |
...
Trigger phrases: "Escambia", "escambia county", "Pensacola", "co_no 17"
