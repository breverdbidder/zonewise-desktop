...
name: county-flagler
description: >
  Zoning intelligence for Flagler County, FL (FDOR co_no: 18).
  ...
  overlay districts. Triggers on: Flagler County, co_no 18,
  any address in Flagler County Florida.
supabase_county_filter: "county=ilike.%25Flagler%25"
co_no: 18
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 18 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Flagler County Florida zoning ordinance municode",
    "Flagler County Florida GIS zoning layer ArcGIS",
    "Flagler County Florida online zoning map portal",
    "site:municode.com Flagler county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'flagler'})
```
...
- Fallback: FDOR fl_parcels table (co_no=18)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-flagler`
