...
name: county-flagler
description: >
  Zoning intelligence for Flagler County, FL.
  5 jurisdictions, FDOR co_no 18.
  Portal: arcgis. Anti-scrape: false.
  Triggers: Flagler, flagler county, Bunnell, co_no 18.
co_no: 18
county_slug: flagler
portal_type: arcgis
anti_scrape: false
...
# Flagler County — Zoning Intelligence

**Seat**: Bunnell | **Pop**: 115,081 | **Municipalities**: 5
**FDOR co_no**: 18 | **Portal**: ARCGIS | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Flagler%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.18&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.18&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Flagler County Florida zoning map"`
2. `"Flagler County Florida municode zoning ordinance"`
3. `"Flagler County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/flagler_county
...
**Container**: `modal-county-flagler`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='flagler'` → Traycer auto-issue.
...
| Municipalities | 5 |
| Portal | ARCGIS |
| Municode | [flagler_county](https://library.municode.com/fl/flagler_county) |
| Anti-scrape | No |
| Rate limit | 40 rpm |
| Phase | P3 |
...
Trigger phrases: "Flagler", "flagler county", "Bunnell", "co_no 18"
