...
description: >
  Zoning intelligence for Franklin County, FL (FDOR co_no: 19).
  ...
  overlay districts. Triggers on: Franklin County, co_no 19,
  any address in Franklin County Florida.
supabase_county_filter: "county=ilike.%25Franklin%25"
co_no: 19
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 19 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Franklin County Florida zoning ordinance municode",
    "Franklin County Florida GIS zoning layer ArcGIS",
    "Franklin County Florida online zoning map portal",
    "site:municode.com Franklin county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'franklin'})
```
...
- Fallback: FDOR fl_parcels table (co_no=19)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-franklin`
