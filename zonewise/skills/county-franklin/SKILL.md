...
description: >
  Zoning intelligence for Franklin County, FL.
  3 jurisdictions, FDOR co_no 19.
  Portal: municode. Anti-scrape: false.
  Triggers: Franklin, franklin county, Apalachicola, co_no 19.
co_no: 19
...
# Franklin County — Zoning Intelligence

**Seat**: Apalachicola | **Pop**: 12,125 | **Municipalities**: 3
**FDOR co_no**: 19 | **Portal**: MUNICODE | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Franklin%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.19&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.19&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Franklin County Florida zoning map"`
2. `"Franklin County Florida municode zoning ordinance"`
3. `"Franklin County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/franklin_county
...
**Container**: `modal-county-franklin`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='franklin'` → Traycer auto-issue.
...
| Municipalities | 3 |
| Portal | MUNICODE |
| Municode | [franklin_county](https://library.municode.com/fl/franklin_county) |
| Anti-scrape | No |
| Rate limit | 20 rpm |
| Phase | P3 |
...
Trigger phrases: "Franklin", "franklin county", "Apalachicola", "co_no 19"
