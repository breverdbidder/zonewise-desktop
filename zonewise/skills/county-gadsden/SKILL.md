...
name: county-gadsden
description: >
  Zoning intelligence for Gadsden County, FL.
  6 jurisdictions, FDOR co_no 20.
  Portal: municode. Anti-scrape: false.
  Triggers: Gadsden, gadsden county, Quincy, co_no 20.
co_no: 20
county_slug: gadsden
...
# Gadsden County — Zoning Intelligence

**Seat**: Quincy | **Pop**: 44,294 | **Municipalities**: 6
**FDOR co_no**: 20 | **Portal**: MUNICODE | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Gadsden%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.20&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.20&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Gadsden County Florida zoning map"`
2. `"Gadsden County Florida municode zoning ordinance"`
3. `"Gadsden County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/gadsden_county
...
**Container**: `modal-county-gadsden`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='gadsden'` → Traycer auto-issue.
...
| Municipalities | 6 |
| Portal | MUNICODE |
| Municode | [gadsden_county](https://library.municode.com/fl/gadsden_county) |
| Anti-scrape | No |
| Rate limit | 20 rpm |
| Phase | P3 |
...
Trigger phrases: "Gadsden", "gadsden county", "Quincy", "co_no 20"
