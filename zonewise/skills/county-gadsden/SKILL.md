...
name: county-gadsden
description: >
  Zoning intelligence for Gadsden County, FL (FDOR co_no: 20).
  ...
  overlay districts. Triggers on: Gadsden County, co_no 20,
  any address in Gadsden County Florida.
supabase_county_filter: "county=ilike.%25Gadsden%25"
co_no: 20
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 20 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Gadsden County Florida zoning ordinance municode",
    "Gadsden County Florida GIS zoning layer ArcGIS",
    "Gadsden County Florida online zoning map portal",
    "site:municode.com Gadsden county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'gadsden'})
```
...
- Fallback: FDOR fl_parcels table (co_no=20)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-gadsden`
