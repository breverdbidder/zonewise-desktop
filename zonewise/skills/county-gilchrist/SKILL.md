...
description: >
  Zoning intelligence for Gilchrist County, FL (FDOR co_no: 21).
  ...
  overlay districts. Triggers on: Gilchrist County, co_no 21,
  any address in Gilchrist County Florida.
supabase_county_filter: "county=ilike.%25Gilchrist%25"
co_no: 21
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 21 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Gilchrist County Florida zoning ordinance municode",
    "Gilchrist County Florida GIS zoning layer ArcGIS",
    "Gilchrist County Florida online zoning map portal",
    "site:municode.com Gilchrist county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'gilchrist'})
```
...
- Fallback: FDOR fl_parcels table (co_no=21)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-gilchrist`
