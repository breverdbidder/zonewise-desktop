...
name: county-glades
description: >
  Zoning intelligence for Glades County, FL (FDOR co_no: 22).
  ...
  overlay districts. Triggers on: Glades County, co_no 22,
  any address in Glades County Florida.
supabase_county_filter: "county=ilike.%25Glades%25"
co_no: 22
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 22 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Glades County Florida zoning ordinance municode",
    "Glades County Florida GIS zoning layer ArcGIS",
    "Glades County Florida online zoning map portal",
    "site:municode.com Glades county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'glades'})
```
...
- Fallback: FDOR fl_parcels table (co_no=22)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-glades`
