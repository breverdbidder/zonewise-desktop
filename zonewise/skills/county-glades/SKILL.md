...
name: county-glades
description: >
  Zoning intelligence for Glades County, FL.
  1 jurisdictions, FDOR co_no 22.
  Portal: pdf. Anti-scrape: false.
  Triggers: Glades, glades county, Moore Haven, co_no 22.
co_no: 22
county_slug: glades
portal_type: pdf
anti_scrape: false
...
# Glades County — Zoning Intelligence

**Seat**: Moore Haven | **Pop**: 13,363 | **Municipalities**: 1
**FDOR co_no**: 22 | **Portal**: PDF | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Glades%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.22&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.22&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Glades County Florida zoning map"`
2. `"Glades County Florida municode zoning ordinance"`
3. `"Glades County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/glades_county
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='glades'` → Traycer auto-issue.
...
| Municipalities | 1 |
| Portal | PDF |
| Municode | [glades_county](https://library.municode.com/fl/glades_county) |
| Anti-scrape | No |
| Rate limit | 10 rpm |
| Phase | P3 |
...
Trigger phrases: "Glades", "glades county", "Moore Haven", "co_no 22"
