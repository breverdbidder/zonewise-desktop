...
name: county-gulf
description: >
  Zoning intelligence for Gulf County, FL.
  3 jurisdictions, FDOR co_no 23.
  Portal: municode. Anti-scrape: false.
  Triggers: Gulf, gulf county, Port St. Joe, co_no 23.
co_no: 23
county_slug: gulf
...
# Gulf County — Zoning Intelligence

**Seat**: Port St. Joe | **Pop**: 17,208 | **Municipalities**: 3
**FDOR co_no**: 23 | **Portal**: MUNICODE | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Gulf%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.23&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.23&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Gulf County Florida zoning map"`
2. `"Gulf County Florida municode zoning ordinance"`
3. `"Gulf County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/gulf_county
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='gulf'` → Traycer auto-issue.
...
| Municipalities | 3 |
| Portal | MUNICODE |
| Municode | [gulf_county](https://library.municode.com/fl/gulf_county) |
| Anti-scrape | No |
| Rate limit | 20 rpm |
| Phase | P3 |
...
Trigger phrases: "Gulf", "gulf county", "Port St. Joe", "co_no 23"
