...
name: county-gulf
description: >
  Zoning intelligence for Gulf County, FL (FDOR co_no: 23).
  ...
  overlay districts. Triggers on: Gulf County, co_no 23,
  any address in Gulf County Florida.
supabase_county_filter: "county=ilike.%25Gulf%25"
co_no: 23
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 23 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Gulf County Florida zoning ordinance municode",
    "Gulf County Florida GIS zoning layer ArcGIS",
    "Gulf County Florida online zoning map portal",
    "site:municode.com Gulf county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'gulf'})
```
...
- Fallback: FDOR fl_parcels table (co_no=23)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-gulf`
