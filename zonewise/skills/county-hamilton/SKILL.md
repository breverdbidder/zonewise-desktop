---
name: county-hamilton
description: >
  Zoning intelligence for Hamilton County, FL.
  3 jurisdictions, FDOR co_no 24.
  Portal: municode. Anti-scrape: false.
  Triggers: Hamilton, hamilton county, Jasper, co_no 24.
co_no: 24
...
# Hamilton County — Zoning Intelligence

**Seat**: Jasper | **Pop**: 14,280 | **Municipalities**: 3
**FDOR co_no**: 24 | **Portal**: MUNICODE | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Hamilton%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.24&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.24&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Hamilton County Florida zoning map"`
2. `"Hamilton County Florida municode zoning ordinance"`
3. `"Hamilton County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/hamilton_county
...
**Container**: `modal-county-hamilton`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='hamilton'` → Traycer auto-issue.
...
| Municipalities | 3 |
| Portal | MUNICODE |
| Municode | [hamilton_county](https://library.municode.com/fl/hamilton_county) |
| Anti-scrape | No |
| Rate limit | 20 rpm |
| Phase | P3 |
...
Trigger phrases: "Hamilton", "hamilton county", "Jasper", "co_no 24"
