...
description: >
  Zoning intelligence for Hamilton County, FL (FDOR co_no: 24).
  ...
  overlay districts. Triggers on: Hamilton County, co_no 24,
  any address in Hamilton County Florida.
supabase_county_filter: "county=ilike.%25Hamilton%25"
co_no: 24
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 24 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Hamilton County Florida zoning ordinance municode",
    "Hamilton County Florida GIS zoning layer ArcGIS",
    "Hamilton County Florida online zoning map portal",
    "site:municode.com Hamilton county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'hamilton'})
```
...
- Fallback: FDOR fl_parcels table (co_no=24)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-hamilton`
