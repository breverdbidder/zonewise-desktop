...
name: county-hardee
description: >
  Zoning intelligence for Hardee County, FL (FDOR co_no: 25).
  ...
  overlay districts. Triggers on: Hardee County, co_no 25,
  any address in Hardee County Florida.
supabase_county_filter: "county=ilike.%25Hardee%25"
co_no: 25
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 25 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Hardee County Florida zoning ordinance municode",
    "Hardee County Florida GIS zoning layer ArcGIS",
    "Hardee County Florida online zoning map portal",
    "site:municode.com Hardee county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'hardee'})
```
...
- Fallback: FDOR fl_parcels table (co_no=25)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-hardee`
