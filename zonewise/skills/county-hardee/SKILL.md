...
name: county-hardee
description: >
  Zoning intelligence for Hardee County, FL.
  3 jurisdictions, FDOR co_no 25.
  Portal: municode. Anti-scrape: false.
  Triggers: Hardee, hardee county, Wauchula, co_no 25.
co_no: 25
county_slug: hardee
...
# Hardee County — Zoning Intelligence

**Seat**: Wauchula | **Pop**: 26,337 | **Municipalities**: 3
**FDOR co_no**: 25 | **Portal**: MUNICODE | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Hardee%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.25&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.25&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Hardee County Florida zoning map"`
2. `"Hardee County Florida municode zoning ordinance"`
3. `"Hardee County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/hardee_county
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='hardee'` → Traycer auto-issue.
...
| Municipalities | 3 |
| Portal | MUNICODE |
| Municode | [hardee_county](https://library.municode.com/fl/hardee_county) |
| Anti-scrape | No |
| Rate limit | 20 rpm |
| Phase | P3 |
...
Trigger phrases: "Hardee", "hardee county", "Wauchula", "co_no 25"
