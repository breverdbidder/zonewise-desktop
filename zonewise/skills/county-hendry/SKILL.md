...
name: county-hendry
description: >
  Zoning intelligence for Hendry County, FL (FDOR co_no: 26).
  ...
  overlay districts. Triggers on: Hendry County, co_no 26,
  any address in Hendry County Florida.
supabase_county_filter: "county=ilike.%25Hendry%25"
co_no: 26
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 26 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Hendry County Florida zoning ordinance municode",
    "Hendry County Florida GIS zoning layer ArcGIS",
    "Hendry County Florida online zoning map portal",
    "site:municode.com Hendry county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'hendry'})
```
...
- Fallback: FDOR fl_parcels table (co_no=26)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-hendry`
