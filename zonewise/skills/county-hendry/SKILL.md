...
name: county-hendry
description: >
  Zoning intelligence for Hendry County, FL.
  2 jurisdictions, FDOR co_no 26.
  Portal: municode. Anti-scrape: false.
  Triggers: Hendry, hendry county, LaBelle, co_no 26.
co_no: 26
county_slug: hendry
...
# Hendry County — Zoning Intelligence

**Seat**: LaBelle | **Pop**: 40,029 | **Municipalities**: 2
**FDOR co_no**: 26 | **Portal**: MUNICODE | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Hendry%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.26&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.26&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Hendry County Florida zoning map"`
2. `"Hendry County Florida municode zoning ordinance"`
3. `"Hendry County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/hendry_county
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='hendry'` → Traycer auto-issue.
...
| Municipalities | 2 |
| Portal | MUNICODE |
| Municode | [hendry_county](https://library.municode.com/fl/hendry_county) |
| Anti-scrape | No |
| Rate limit | 20 rpm |
| Phase | P3 |
...
Trigger phrases: "Hendry", "hendry county", "LaBelle", "co_no 26"
