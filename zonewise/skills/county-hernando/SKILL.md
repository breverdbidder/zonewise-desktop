...
description: >
  Zoning intelligence for Hernando County, FL (FDOR co_no: 27).
  ...
  overlay districts. Triggers on: Hernando County, co_no 27,
  any address in Hernando County Florida.
supabase_county_filter: "county=ilike.%25Hernando%25"
co_no: 27
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 27 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Hernando County Florida zoning ordinance municode",
    "Hernando County Florida GIS zoning layer ArcGIS",
    "Hernando County Florida online zoning map portal",
    "site:municode.com Hernando county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'hernando'})
```
...
- Fallback: FDOR fl_parcels table (co_no=27)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-hernando`
