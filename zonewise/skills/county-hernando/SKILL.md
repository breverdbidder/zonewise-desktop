---
name: county-hernando
description: >
  Zoning intelligence for Hernando County, FL.
  2 jurisdictions, FDOR co_no 27.
  Portal: arcgis. Anti-scrape: false.
  Triggers: Hernando, hernando county, Brooksville, co_no 27.
co_no: 27
...
# Hernando County — Zoning Intelligence

**Seat**: Brooksville | **Pop**: 197,908 | **Municipalities**: 2
**FDOR co_no**: 27 | **Portal**: ARCGIS | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Hernando%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.27&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.27&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Hernando County Florida zoning map"`
2. `"Hernando County Florida municode zoning ordinance"`
3. `"Hernando County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/hernando_county
...
**Container**: `modal-county-hernando`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='hernando'` → Traycer auto-issue.
...
| Municipalities | 2 |
| Portal | ARCGIS |
| Municode | [hernando_county](https://library.municode.com/fl/hernando_county) |
| Anti-scrape | No |
| Rate limit | 40 rpm |
| Phase | P3 |
...
Trigger phrases: "Hernando", "hernando county", "Brooksville", "co_no 27"
