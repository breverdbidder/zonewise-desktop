...
description: >
  Zoning intelligence for Hillsborough County, FL (FDOR co_no: 29).
  ...
  overlay districts. Triggers on: Hillsborough County, co_no 29,
  any address in Hillsborough County Florida.
supabase_county_filter: "county=ilike.%25Hillsborough%25"
co_no: 29
portal_type: unknown
anti_scrape: true
rate_limit_rpm: 10
...
> **co_no**: 29 | **Phase**: P1 | **Anti-scrape**: true | **Rate limit**: 10 rpm
...
```
GET /jurisdictions
  ?county=ilike.%25Hillsborough%25
...
```python
queries = [
    "Hillsborough County Florida zoning ordinance municode",
    "Hillsborough County Florida GIS zoning layer ArcGIS",
    "Hillsborough County Florida online zoning map portal",
    "site:municode.com Hillsborough county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'hillsborough'})
```
...
- Fallback: FDOR fl_parcels table (co_no=29)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-hillsborough`
