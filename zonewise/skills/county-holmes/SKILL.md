...
name: county-holmes
description: >
  Zoning intelligence for Holmes County, FL (FDOR co_no: 30).
  ...
  overlay districts. Triggers on: Holmes County, co_no 30,
  any address in Holmes County Florida.
supabase_county_filter: "county=ilike.%25Holmes%25"
co_no: 30
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 30 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Holmes County Florida zoning ordinance municode",
    "Holmes County Florida GIS zoning layer ArcGIS",
    "Holmes County Florida online zoning map portal",
    "site:municode.com Holmes county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'holmes'})
```
...
- Fallback: FDOR fl_parcels table (co_no=30)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-holmes`
