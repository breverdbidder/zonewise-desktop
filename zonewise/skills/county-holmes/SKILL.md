...
name: county-holmes
description: >
  Zoning intelligence for Holmes County, FL.
  4 jurisdictions, FDOR co_no 30.
  Portal: pdf. Anti-scrape: false.
  Triggers: Holmes, holmes county, Bonifay, co_no 30.
co_no: 30
county_slug: holmes
portal_type: pdf
anti_scrape: false
...
# Holmes County — Zoning Intelligence

**Seat**: Bonifay | **Pop**: 19,952 | **Municipalities**: 4
**FDOR co_no**: 30 | **Portal**: PDF | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Holmes%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.30&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.30&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Holmes County Florida zoning map"`
2. `"Holmes County Florida municode zoning ordinance"`
3. `"Holmes County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/holmes_county
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='holmes'` → Traycer auto-issue.
...
| Municipalities | 4 |
| Portal | PDF |
| Municode | [holmes_county](https://library.municode.com/fl/holmes_county) |
| Anti-scrape | No |
| Rate limit | 10 rpm |
| Phase | P3 |
...
Trigger phrases: "Holmes", "holmes county", "Bonifay", "co_no 30"
