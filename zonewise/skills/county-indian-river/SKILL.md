---
name: county-indian-river
description: >
  Zoning intelligence for Indian River County, FL.
  5 jurisdictions, FDOR co_no 31.
  Portal: arcgis. Anti-scrape: false.
  Triggers: Indian River, indian river county, Vero Beach, co_no 31.
co_no: 31
county_slug: indian-river
portal_type: arcgis
anti_scrape: false
rate_limit_rpm: 60
phase: P3
...
# Indian River County — Zoning Intelligence

**Seat**: Vero Beach | **Pop**: 159,923 | **Municipalities**: 5
**FDOR co_no**: 31 | **Portal**: ARCGIS | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Indian River%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.31&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.31&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Indian River County Florida zoning map"`
2. `"Indian River County Florida municode zoning ordinance"`
3. `"Indian River County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/indian_river_county
...
**Container**: `modal-county-indian-river`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='indian-river'` → Traycer auto-issue.
...
| Municipalities | 5 |
| Portal | ARCGIS |
| Municode | [indian_river_county](https://library.municode.com/fl/indian_river_county) |
| Anti-scrape | No |
| Rate limit | 60 rpm |
| Phase | P3 |
...
Trigger phrases: "Indian River", "indian river county", "Vero Beach", "co_no 31"
