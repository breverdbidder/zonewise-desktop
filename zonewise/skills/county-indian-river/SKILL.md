...
description: >
  Zoning intelligence for Indian River County, FL (FDOR co_no: 31).
  ...
  overlay districts. Triggers on: Indian River County, co_no 31,
  any address in Indian River County Florida.
supabase_county_filter: "county=ilike.%25Indian River%25"
co_no: 31
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 31 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```
GET /jurisdictions
  ?county=ilike.%25Indian River%25
...
```python
queries = [
    "Indian River County Florida zoning ordinance municode",
    "Indian River County Florida GIS zoning layer ArcGIS",
    "Indian River County Florida online zoning map portal",
    "site:municode.com Indian River county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'indian-river'})
```
...
- Fallback: FDOR fl_parcels table (co_no=31)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-indian-river`
