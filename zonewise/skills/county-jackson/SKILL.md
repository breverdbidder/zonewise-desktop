...
name: county-jackson
description: >
  Zoning intelligence for Jackson County, FL (FDOR co_no: 32).
  ...
  overlay districts. Triggers on: Jackson County, co_no 32,
  any address in Jackson County Florida.
supabase_county_filter: "county=ilike.%25Jackson%25"
co_no: 32
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 32 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Jackson County Florida zoning ordinance municode",
    "Jackson County Florida GIS zoning layer ArcGIS",
    "Jackson County Florida online zoning map portal",
    "site:municode.com Jackson county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'jackson'})
```
...
- Fallback: FDOR fl_parcels table (co_no=32)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-jackson`
