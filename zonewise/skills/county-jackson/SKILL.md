...
name: county-jackson
description: >
  Zoning intelligence for Jackson County, FL.
  7 jurisdictions, FDOR co_no 32.
  Portal: municode. Anti-scrape: false.
  Triggers: Jackson, jackson county, Marianna, co_no 32.
co_no: 32
county_slug: jackson
...
# Jackson County — Zoning Intelligence

**Seat**: Marianna | **Pop**: 46,414 | **Municipalities**: 7
**FDOR co_no**: 32 | **Portal**: MUNICODE | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Jackson%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.32&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.32&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Jackson County Florida zoning map"`
2. `"Jackson County Florida municode zoning ordinance"`
3. `"Jackson County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/jackson_county
...
**Container**: `modal-county-jackson`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='jackson'` → Traycer auto-issue.
...
| Municipalities | 7 |
| Portal | MUNICODE |
| Municode | [jackson_county](https://library.municode.com/fl/jackson_county) |
| Anti-scrape | No |
| Rate limit | 20 rpm |
| Phase | P3 |
...
Trigger phrases: "Jackson", "jackson county", "Marianna", "co_no 32"
