...
description: >
  Zoning intelligence for Jefferson County, FL (FDOR co_no: 33).
  ...
  overlay districts. Triggers on: Jefferson County, co_no 33,
  any address in Jefferson County Florida.
supabase_county_filter: "county=ilike.%25Jefferson%25"
co_no: 33
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 33 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Jefferson County Florida zoning ordinance municode",
    "Jefferson County Florida GIS zoning layer ArcGIS",
    "Jefferson County Florida online zoning map portal",
    "site:municode.com Jefferson county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'jefferson'})
```
...
- Fallback: FDOR fl_parcels table (co_no=33)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-jefferson`
