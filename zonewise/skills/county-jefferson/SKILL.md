---
name: county-jefferson
description: >
  Zoning intelligence for Jefferson County, FL.
  1 jurisdictions, FDOR co_no 33.
  Portal: pdf. Anti-scrape: false.
  Triggers: Jefferson, jefferson county, Monticello, co_no 33.
co_no: 33
...
# Jefferson County — Zoning Intelligence

**Seat**: Monticello | **Pop**: 14,716 | **Municipalities**: 1
**FDOR co_no**: 33 | **Portal**: PDF | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Jefferson%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.33&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.33&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Jefferson County Florida zoning map"`
2. `"Jefferson County Florida municode zoning ordinance"`
3. `"Jefferson County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/jefferson_county
...
**Rate limit**: 10 rpm | **Anti-detect**: DISABLED
**Container**: `modal-county-jefferson`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='jefferson'` → Traycer auto-issue.
...
| Municipalities | 1 |
| Portal | PDF |
| Municode | [jefferson_county](https://library.municode.com/fl/jefferson_county) |
| Anti-scrape | No |
| Rate limit | 10 rpm |
| Phase | P3 |
...
Trigger phrases: "Jefferson", "jefferson county", "Monticello", "co_no 33"
