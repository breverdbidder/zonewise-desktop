...
description: >
  Zoning intelligence for Lafayette County, FL (FDOR co_no: 34).
  ...
  overlay districts. Triggers on: Lafayette County, co_no 34,
  any address in Lafayette County Florida.
supabase_county_filter: "county=ilike.%25Lafayette%25"
co_no: 34
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 34 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Lafayette County Florida zoning ordinance municode",
    "Lafayette County Florida GIS zoning layer ArcGIS",
    "Lafayette County Florida online zoning map portal",
    "site:municode.com Lafayette county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'lafayette'})
```
...
- Fallback: FDOR fl_parcels table (co_no=34)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-lafayette`
