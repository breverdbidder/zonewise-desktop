---
name: county-lafayette
description: >
  Zoning intelligence for Lafayette County, FL.
  1 jurisdictions, FDOR co_no 34.
  Portal: pdf. Anti-scrape: false.
  Triggers: Lafayette, lafayette county, Mayo, co_no 34.
co_no: 34
...
# Lafayette County — Zoning Intelligence

**Seat**: Mayo | **Pop**: 8,924 | **Municipalities**: 1
**FDOR co_no**: 34 | **Portal**: PDF | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Lafayette%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.34&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.34&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Lafayette County Florida zoning map"`
2. `"Lafayette County Florida municode zoning ordinance"`
3. `"Lafayette County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/lafayette_county
...
**Container**: `modal-county-lafayette`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='lafayette'` → Traycer auto-issue.
...
| Municipalities | 1 |
| Portal | PDF |
| Municode | [lafayette_county](https://library.municode.com/fl/lafayette_county) |
| Anti-scrape | No |
| Rate limit | 10 rpm |
| Phase | P3 |
...
Trigger phrases: "Lafayette", "lafayette county", "Mayo", "co_no 34"
