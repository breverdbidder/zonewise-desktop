...
name: county-lake
description: >
  Zoning intelligence for Lake County, FL.
  14 jurisdictions, FDOR co_no 35.
  Portal: arcgis. Anti-scrape: false.
  Triggers: Lake, lake county, Tavares, co_no 35.
co_no: 35
county_slug: lake
portal_type: arcgis
anti_scrape: false
...
# Lake County — Zoning Intelligence

**Seat**: Tavares | **Pop**: 385,240 | **Municipalities**: 14
**FDOR co_no**: 35 | **Portal**: ARCGIS | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Lake%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.35&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.35&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Lake County Florida zoning map"`
2. `"Lake County Florida municode zoning ordinance"`
3. `"Lake County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/lake_county
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='lake'` → Traycer auto-issue.
...
| Municipalities | 14 |
| Portal | ARCGIS |
| Municode | [lake_county](https://library.municode.com/fl/lake_county) |
| Anti-scrape | No |
| Rate limit | 60 rpm |
| Phase | P3 |
...
Trigger phrases: "Lake", "lake county", "Tavares", "co_no 35"
