...
name: county-lake
description: >
  Zoning intelligence for Lake County, FL (FDOR co_no: 35).
  ...
  overlay districts. Triggers on: Lake County, co_no 35,
  any address in Lake County Florida.
supabase_county_filter: "county=ilike.%25Lake%25"
co_no: 35
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 35 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Lake County Florida zoning ordinance municode",
    "Lake County Florida GIS zoning layer ArcGIS",
    "Lake County Florida online zoning map portal",
    "site:municode.com Lake county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'lake'})
```
...
- Fallback: FDOR fl_parcels table (co_no=35)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-lake`
