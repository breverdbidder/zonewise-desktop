...
name: county-lee
description: >
  Zoning intelligence for Lee County, FL.
  6 jurisdictions, FDOR co_no 36.
  Portal: arcgis. Anti-scrape: false.
  Triggers: Lee, lee county, Fort Myers, co_no 36.
co_no: 36
county_slug: lee
portal_type: arcgis
anti_scrape: false
...
# Lee County — Zoning Intelligence

**Seat**: Fort Myers | **Pop**: 760,822 | **Municipalities**: 6
**FDOR co_no**: 36 | **Portal**: ARCGIS | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Lee%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.36&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.36&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Lee County Florida zoning map"`
2. `"Lee County Florida municode zoning ordinance"`
3. `"Lee County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/lee_county
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='lee'` → Traycer auto-issue.
...
| Municipalities | 6 |
| Portal | ARCGIS |
| Municode | [lee_county](https://library.municode.com/fl/lee_county) |
| Anti-scrape | No |
| Rate limit | 60 rpm |
| Phase | P3 |
...
Trigger phrases: "Lee", "lee county", "Fort Myers", "co_no 36"
