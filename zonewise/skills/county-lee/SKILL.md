...
name: county-lee
description: >
  Zoning intelligence for Lee County, FL (FDOR co_no: 36).
  ...
  overlay districts. Triggers on: Lee County, co_no 36,
  any address in Lee County Florida.
supabase_county_filter: "county=ilike.%25Lee%25"
co_no: 36
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 36 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Lee County Florida zoning ordinance municode",
    "Lee County Florida GIS zoning layer ArcGIS",
    "Lee County Florida online zoning map portal",
    "site:municode.com Lee county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'lee'})
```
...
- Fallback: FDOR fl_parcels table (co_no=36)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-lee`
