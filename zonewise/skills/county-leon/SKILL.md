...
name: county-leon
description: >
  Zoning intelligence for Leon County, FL (FDOR co_no: 37).
  ...
  overlay districts. Triggers on: Leon County, co_no 37,
  any address in Leon County Florida.
supabase_county_filter: "county=ilike.%25Leon%25"
co_no: 37
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 37 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Leon County Florida zoning ordinance municode",
    "Leon County Florida GIS zoning layer ArcGIS",
    "Leon County Florida online zoning map portal",
    "site:municode.com Leon county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'leon'})
```
...
- Fallback: FDOR fl_parcels table (co_no=37)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-leon`
