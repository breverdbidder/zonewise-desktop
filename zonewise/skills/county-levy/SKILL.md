...
name: county-levy
description: >
  Zoning intelligence for Levy County, FL.
  6 jurisdictions, FDOR co_no 38.
  Portal: municode. Anti-scrape: false.
  Triggers: Levy, levy county, Bronson, co_no 38.
co_no: 38
county_slug: levy
...
# Levy County — Zoning Intelligence

**Seat**: Bronson | **Pop**: 42,557 | **Municipalities**: 6
**FDOR co_no**: 38 | **Portal**: MUNICODE | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Levy%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.38&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.38&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Levy County Florida zoning map"`
2. `"Levy County Florida municode zoning ordinance"`
3. `"Levy County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/levy_county
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='levy'` → Traycer auto-issue.
...
| Municipalities | 6 |
| Portal | MUNICODE |
| Municode | [levy_county](https://library.municode.com/fl/levy_county) |
| Anti-scrape | No |
| Rate limit | 20 rpm |
| Phase | P3 |
...
Trigger phrases: "Levy", "levy county", "Bronson", "co_no 38"
