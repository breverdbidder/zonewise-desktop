...
name: county-liberty
description: >
  Zoning intelligence for Liberty County, FL.
  1 jurisdictions, FDOR co_no 39.
  Portal: pdf. Anti-scrape: false.
  Triggers: Liberty, liberty county, Bristol, co_no 39.
co_no: 39
county_slug: liberty
portal_type: pdf
anti_scrape: false
...
**Seat**: Bristol | **Pop**: 7,994 | **Municipalities**: 1
**FDOR co_no**: 39 | **Portal**: PDF | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Liberty%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.39&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.39&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Liberty County Florida zoning map"`
2. `"Liberty County Florida municode zoning ordinance"`
3. `"Liberty County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/liberty_county
...
**Container**: `modal-county-liberty`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='liberty'` → Traycer auto-issue.
...
| Municipalities | 1 |
| Portal | PDF |
| Municode | [liberty_county](https://library.municode.com/fl/liberty_county) |
| Anti-scrape | No |
| Rate limit | 10 rpm |
| Phase | P3 |
...
Trigger phrases: "Liberty", "liberty county", "Bristol", "co_no 39"
