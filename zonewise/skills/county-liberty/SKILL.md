...
name: county-liberty
description: >
  Zoning intelligence for Liberty County, FL (FDOR co_no: 39).
  ...
  overlay districts. Triggers on: Liberty County, co_no 39,
  any address in Liberty County Florida.
supabase_county_filter: "county=ilike.%25Liberty%25"
co_no: 39
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
# Liberty County — Zoning Intelligence

> **co_no**: 39 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Liberty County Florida zoning ordinance municode",
    "Liberty County Florida GIS zoning layer ArcGIS",
    "Liberty County Florida online zoning map portal",
    "site:municode.com Liberty county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'liberty'})
```
...
- Fallback: FDOR fl_parcels table (co_no=39)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-liberty`
