...
name: county-madison
description: >
  Zoning intelligence for Madison County, FL.
  4 jurisdictions, FDOR co_no 40.
  Portal: municode. Anti-scrape: false.
  Triggers: Madison, madison county, Madison, co_no 40.
co_no: 40
county_slug: madison
...
**Seat**: Madison | **Pop**: 18,474 | **Municipalities**: 4
**FDOR co_no**: 40 | **Portal**: MUNICODE | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Madison%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.40&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.40&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Madison County Florida zoning map"`
2. `"Madison County Florida municode zoning ordinance"`
3. `"Madison County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/madison_county
...
**Container**: `modal-county-madison`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='madison'` → Traycer auto-issue.
...
| Municipalities | 4 |
| Portal | MUNICODE |
| Municode | [madison_county](https://library.municode.com/fl/madison_county) |
| Anti-scrape | No |
| Rate limit | 20 rpm |
| Phase | P3 |
...
Trigger phrases: "Madison", "madison county", "Madison", "co_no 40"
