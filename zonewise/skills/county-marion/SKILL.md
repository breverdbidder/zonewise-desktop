...
name: county-marion
description: >
  Zoning intelligence for Marion County, FL (FDOR co_no: 42).
  ...
  overlay districts. Triggers on: Marion County, co_no 42,
  any address in Marion County Florida.
supabase_county_filter: "county=ilike.%25Marion%25"
co_no: 42
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 42 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Marion County Florida zoning ordinance municode",
    "Marion County Florida GIS zoning layer ArcGIS",
    "Marion County Florida online zoning map portal",
    "site:municode.com Marion county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'marion'})
```
...
- Fallback: FDOR fl_parcels table (co_no=42)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-marion`
