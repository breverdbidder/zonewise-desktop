...
name: county-martin
description: >
  Zoning intelligence for Martin County, FL (FDOR co_no: 43).
  ...
  overlay districts. Triggers on: Martin County, co_no 43,
  any address in Martin County Florida.
supabase_county_filter: "county=ilike.%25Martin%25"
co_no: 43
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 43 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Martin County Florida zoning ordinance municode",
    "Martin County Florida GIS zoning layer ArcGIS",
    "Martin County Florida online zoning map portal",
    "site:municode.com Martin county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'martin'})
```
...
- Fallback: FDOR fl_parcels table (co_no=43)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-martin`
