...
name: county-martin
description: >
  Zoning intelligence for Martin County, FL.
  4 jurisdictions, FDOR co_no 43.
  Portal: arcgis. Anti-scrape: false.
  Triggers: Martin, martin county, Stuart, co_no 43.
co_no: 43
county_slug: martin
portal_type: arcgis
anti_scrape: false
...
# Martin County — Zoning Intelligence

**Seat**: Stuart | **Pop**: 160,998 | **Municipalities**: 4
**FDOR co_no**: 43 | **Portal**: ARCGIS | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Martin%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.43&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.43&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Martin County Florida zoning map"`
2. `"Martin County Florida municode zoning ordinance"`
3. `"Martin County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/martin_county
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='martin'` → Traycer auto-issue.
...
| Municipalities | 4 |
| Portal | ARCGIS |
| Municode | [martin_county](https://library.municode.com/fl/martin_county) |
| Anti-scrape | No |
| Rate limit | 40 rpm |
| Phase | P3 |
...
Trigger phrases: "Martin", "martin county", "Stuart", "co_no 43"
