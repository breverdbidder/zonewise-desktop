...
name: county-monroe
description: >
  Zoning intelligence for Monroe County, FL.
  3 jurisdictions, FDOR co_no 44.
  Portal: arcgis. Anti-scrape: true.
  Triggers: Monroe, monroe county, Key West, co_no 44.
co_no: 44
county_slug: monroe
portal_type: arcgis
anti_scrape: true
...
# Monroe County — Zoning Intelligence

**Seat**: Key West | **Pop**: 82,874 | **Municipalities**: 3
**FDOR co_no**: 44 | **Portal**: ARCGIS | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Monroe%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.44&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.44&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Monroe County Florida zoning map"`
2. `"Monroe County Florida municode zoning ordinance"`
3. `"Monroe County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/monroe_county
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='monroe'` → Traycer auto-issue.
...
| Municipalities | 3 |
| Portal | ARCGIS |
| Municode | [monroe_county](https://library.municode.com/fl/monroe_county) |
| Anti-scrape | ⚠️ YES |
| Rate limit | 30 rpm |
| Phase | P3 |
...
Trigger phrases: "Monroe", "monroe county", "Key West", "co_no 44"
