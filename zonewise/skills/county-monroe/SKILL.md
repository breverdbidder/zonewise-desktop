...
name: county-monroe
description: >
  Zoning intelligence for Monroe County, FL (FDOR co_no: 44).
  ...
  overlay districts. Triggers on: Monroe County, co_no 44,
  any address in Monroe County Florida.
supabase_county_filter: "county=ilike.%25Monroe%25"
co_no: 44
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 44 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Monroe County Florida zoning ordinance municode",
    "Monroe County Florida GIS zoning layer ArcGIS",
    "Monroe County Florida online zoning map portal",
    "site:municode.com Monroe county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'monroe'})
```
...
- Fallback: FDOR fl_parcels table (co_no=44)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-monroe`
