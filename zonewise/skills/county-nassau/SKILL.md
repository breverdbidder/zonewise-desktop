...
name: county-nassau
description: >
  Zoning intelligence for Nassau County, FL (FDOR co_no: 45).
  ...
  overlay districts. Triggers on: Nassau County, co_no 45,
  any address in Nassau County Florida.
supabase_county_filter: "county=ilike.%25Nassau%25"
co_no: 45
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 45 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Nassau County Florida zoning ordinance municode",
    "Nassau County Florida GIS zoning layer ArcGIS",
    "Nassau County Florida online zoning map portal",
    "site:municode.com Nassau county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'nassau'})
```
...
- Fallback: FDOR fl_parcels table (co_no=45)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-nassau`
