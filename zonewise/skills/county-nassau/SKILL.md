...
name: county-nassau
description: >
  Zoning intelligence for Nassau County, FL.
  3 jurisdictions, FDOR co_no 45.
  Portal: arcgis. Anti-scrape: false.
  Triggers: Nassau, nassau county, Fernandina Beach, co_no 45.
co_no: 45
county_slug: nassau
portal_type: arcgis
anti_scrape: false
...
# Nassau County — Zoning Intelligence

**Seat**: Fernandina Beach | **Pop**: 90,743 | **Municipalities**: 3
**FDOR co_no**: 45 | **Portal**: ARCGIS | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Nassau%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.45&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.45&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Nassau County Florida zoning map"`
2. `"Nassau County Florida municode zoning ordinance"`
3. `"Nassau County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/nassau_county
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='nassau'` → Traycer auto-issue.
...
| Municipalities | 3 |
| Portal | ARCGIS |
| Municode | [nassau_county](https://library.municode.com/fl/nassau_county) |
| Anti-scrape | No |
| Rate limit | 40 rpm |
| Phase | P3 |
...
Trigger phrases: "Nassau", "nassau county", "Fernandina Beach", "co_no 45"
