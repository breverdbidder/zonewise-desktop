---
name: county-okaloosa
description: >
  Zoning intelligence for Okaloosa County, FL.
  9 jurisdictions, FDOR co_no 46.
  Portal: arcgis. Anti-scrape: false.
  Triggers: Okaloosa, okaloosa county, Crestview, co_no 46.
co_no: 46
...
# Okaloosa County — Zoning Intelligence

**Seat**: Crestview | **Pop**: 216,803 | **Municipalities**: 9
**FDOR co_no**: 46 | **Portal**: ARCGIS | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Okaloosa%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.46&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.46&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Okaloosa County Florida zoning map"`
2. `"Okaloosa County Florida municode zoning ordinance"`
3. `"Okaloosa County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/okaloosa_county
...
**Container**: `modal-county-okaloosa`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='okaloosa'` → Traycer auto-issue.
...
| Municipalities | 9 |
| Portal | ARCGIS |
| Municode | [okaloosa_county](https://library.municode.com/fl/okaloosa_county) |
| Anti-scrape | No |
| Rate limit | 40 rpm |
| Phase | P3 |
...
Trigger phrases: "Okaloosa", "okaloosa county", "Crestview", "co_no 46"
