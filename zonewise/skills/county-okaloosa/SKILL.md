...
description: >
  Zoning intelligence for Okaloosa County, FL (FDOR co_no: 46).
  ...
  overlay districts. Triggers on: Okaloosa County, co_no 46,
  any address in Okaloosa County Florida.
supabase_county_filter: "county=ilike.%25Okaloosa%25"
co_no: 46
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 46 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Okaloosa County Florida zoning ordinance municode",
    "Okaloosa County Florida GIS zoning layer ArcGIS",
    "Okaloosa County Florida online zoning map portal",
    "site:municode.com Okaloosa county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'okaloosa'})
```
...
- Fallback: FDOR fl_parcels table (co_no=46)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-okaloosa`
