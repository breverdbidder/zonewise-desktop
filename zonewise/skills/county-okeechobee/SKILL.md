...
description: >
  Zoning intelligence for Okeechobee County, FL.
  1 jurisdictions, FDOR co_no 47.
  Portal: municode. Anti-scrape: false.
  Triggers: Okeechobee, okeechobee county, Okeechobee, co_no 47.
co_no: 47
...
# Okeechobee County — Zoning Intelligence

**Seat**: Okeechobee | **Pop**: 41,764 | **Municipalities**: 1
**FDOR co_no**: 47 | **Portal**: MUNICODE | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Okeechobee%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.47&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.47&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Okeechobee County Florida zoning map"`
2. `"Okeechobee County Florida municode zoning ordinance"`
3. `"Okeechobee County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/okeechobee_county
...
**Container**: `modal-county-okeechobee`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='okeechobee'` → Traycer auto-issue.
...
| Municipalities | 1 |
| Portal | MUNICODE |
| Municode | [okeechobee_county](https://library.municode.com/fl/okeechobee_county) |
| Anti-scrape | No |
| Rate limit | 20 rpm |
| Phase | P3 |
...
Trigger phrases: "Okeechobee", "okeechobee county", "Okeechobee", "co_no 47"
