...
name: county-osceola
description: >
  Zoning intelligence for Osceola County, FL (FDOR co_no: 49).
  ...
  overlay districts. Triggers on: Osceola County, co_no 49,
  any address in Osceola County Florida.
supabase_county_filter: "county=ilike.%25Osceola%25"
co_no: 49
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 49 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Osceola County Florida zoning ordinance municode",
    "Osceola County Florida GIS zoning layer ArcGIS",
    "Osceola County Florida online zoning map portal",
    "site:municode.com Osceola county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'osceola'})
```
...
- Fallback: FDOR fl_parcels table (co_no=49)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-osceola`
