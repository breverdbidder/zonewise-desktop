...
name: county-osceola
description: >
  Zoning intelligence for Osceola County, FL.
  3 jurisdictions, FDOR co_no 49.
  Portal: arcgis. Anti-scrape: false.
  Triggers: Osceola, osceola county, Kissimmee, co_no 49.
co_no: 49
county_slug: osceola
portal_type: arcgis
anti_scrape: false
...
# Osceola County — Zoning Intelligence

**Seat**: Kissimmee | **Pop**: 388,656 | **Municipalities**: 3
**FDOR co_no**: 49 | **Portal**: ARCGIS | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Osceola%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.49&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.49&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Osceola County Florida zoning map"`
2. `"Osceola County Florida municode zoning ordinance"`
3. `"Osceola County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/osceola_county
...
**Container**: `modal-county-osceola`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='osceola'` → Traycer auto-issue.
...
| Municipalities | 3 |
| Portal | ARCGIS |
| Municode | [osceola_county](https://library.municode.com/fl/osceola_county) |
| Anti-scrape | No |
| Rate limit | 40 rpm |
| Phase | P3 |
...
Trigger phrases: "Osceola", "osceola county", "Kissimmee", "co_no 49"
