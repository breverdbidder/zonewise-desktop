...
description: >
  Zoning intelligence for Palm Beach County, FL (FDOR co_no: 50).
  ...
  overlay districts. Triggers on: Palm Beach County, co_no 50,
  any address in Palm Beach County Florida.
supabase_county_filter: "county=ilike.%25Palm Beach%25"
co_no: 50
portal_type: unknown
anti_scrape: true
rate_limit_rpm: 10
...
> **co_no**: 50 | **Phase**: P1 | **Anti-scrape**: true | **Rate limit**: 10 rpm
...
```python
queries = [
    "Palm Beach County Florida zoning ordinance municode",
    "Palm Beach County Florida GIS zoning layer ArcGIS",
    "Palm Beach County Florida online zoning map portal",
    "site:municode.com Palm Beach county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'palm-beach'})
```
...
- Fallback: FDOR fl_parcels table (co_no=50)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-palm-beach`
