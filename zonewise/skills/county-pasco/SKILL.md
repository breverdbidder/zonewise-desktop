...
name: county-pasco
description: >
  Zoning intelligence for Pasco County, FL (FDOR co_no: 51).
  ...
  overlay districts. Triggers on: Pasco County, co_no 51,
  any address in Pasco County Florida.
supabase_county_filter: "county=ilike.%25Pasco%25"
co_no: 51
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 51 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Pasco County Florida zoning ordinance municode",
    "Pasco County Florida GIS zoning layer ArcGIS",
    "Pasco County Florida online zoning map portal",
    "site:municode.com Pasco county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'pasco'})
```
...
- Fallback: FDOR fl_parcels table (co_no=51)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-pasco`
