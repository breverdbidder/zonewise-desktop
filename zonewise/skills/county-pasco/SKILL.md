...
name: county-pasco
description: >
  Zoning intelligence for Pasco County, FL.
  4 jurisdictions, FDOR co_no 51.
  Portal: arcgis. Anti-scrape: false.
  Triggers: Pasco, pasco county, Dade City, co_no 51.
co_no: 51
county_slug: pasco
portal_type: arcgis
anti_scrape: false
...
# Pasco County — Zoning Intelligence

**Seat**: Dade City | **Pop**: 561,891 | **Municipalities**: 4
**FDOR co_no**: 51 | **Portal**: ARCGIS | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Pasco%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.51&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.51&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Pasco County Florida zoning map"`
2. `"Pasco County Florida municode zoning ordinance"`
3. `"Pasco County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/pasco_county
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='pasco'` → Traycer auto-issue.
...
| Municipalities | 4 |
| Portal | ARCGIS |
| Municode | [pasco_county](https://library.municode.com/fl/pasco_county) |
| Anti-scrape | No |
| Rate limit | 40 rpm |
| Phase | P3 |
...
Trigger phrases: "Pasco", "pasco county", "Dade City", "co_no 51"
