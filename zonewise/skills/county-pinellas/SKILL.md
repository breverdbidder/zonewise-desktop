...
description: >
  Zoning intelligence for Pinellas County, FL.
  24 jurisdictions, FDOR co_no 52.
  Portal: arcgis. Anti-scrape: true.
  Triggers: Pinellas, pinellas county, Clearwater, co_no 52.
co_no: 52
...
**Seat**: Clearwater | **Pop**: 959,107 | **Municipalities**: 24
**FDOR co_no**: 52 | **Portal**: ARCGIS | **Phase**: P1
...
```
GET /jurisdictions?county=ilike.%25Pinellas%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.52&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.52&parcel_id=eq.{parcel_id}&select=*
```

### GIS Direct Query (ArcGIS)
```
GET https://egis.pinellas.gov/arcgis/rest/services/Zoning/MapServer/query?where=1=1&geometry={lng},{lat}&geometryType=esriGeometryPoint&spatialRel=esriSpatialRelIntersects&outFields=ZONE_CODE&f=json
```
...
Queries:
1. `"Pinellas County Florida zoning map"`
2. `"Pinellas County Florida municode zoning ordinance"`
3. `"Pinellas County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/pinellas_county
...
**Container**: `modal-county-pinellas`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='pinellas'` → Traycer auto-issue.
...
| Municipalities | 24 |
| Portal | ARCGIS |
| Municode | [pinellas_county](https://library.municode.com/fl/pinellas_county) |
| Anti-scrape | ⚠️ YES |
| Rate limit | 20 rpm |
| Phase | P1 |
...
Trigger phrases: "Pinellas", "pinellas county", "Clearwater", "co_no 52"
