...
name: county-polk
description: >
  Zoning intelligence for Polk County, FL (FDOR co_no: 53).
  ...
  overlay districts. Triggers on: Polk County, co_no 53,
  any address in Polk County Florida.
supabase_county_filter: "county=ilike.%25Polk%25"
co_no: 53
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 53 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Polk County Florida zoning ordinance municode",
    "Polk County Florida GIS zoning layer ArcGIS",
    "Polk County Florida online zoning map portal",
    "site:municode.com Polk county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'polk'})
```
...
- Fallback: FDOR fl_parcels table (co_no=53)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-polk`
