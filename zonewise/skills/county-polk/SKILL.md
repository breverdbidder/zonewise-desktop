...
name: county-polk
description: >
  Zoning intelligence for Polk County, FL.
  18 jurisdictions, FDOR co_no 53.
  Portal: arcgis. Anti-scrape: false.
  Triggers: Polk, polk county, Bartow, co_no 53.
co_no: 53
county_slug: polk
portal_type: arcgis
anti_scrape: false
...
# Polk County — Zoning Intelligence

**Seat**: Bartow | **Pop**: 724,777 | **Municipalities**: 18
**FDOR co_no**: 53 | **Portal**: ARCGIS | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Polk%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.53&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.53&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Polk County Florida zoning map"`
2. `"Polk County Florida municode zoning ordinance"`
3. `"Polk County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/polk_county
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='polk'` → Traycer auto-issue.
...
| Municipalities | 18 |
| Portal | ARCGIS |
| Municode | [polk_county](https://library.municode.com/fl/polk_county) |
| Anti-scrape | No |
| Rate limit | 40 rpm |
| Phase | P3 |
...
Trigger phrases: "Polk", "polk county", "Bartow", "co_no 53"
