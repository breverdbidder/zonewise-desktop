...
name: county-putnam
description: >
  Zoning intelligence for Putnam County, FL (FDOR co_no: 54).
  ...
  overlay districts. Triggers on: Putnam County, co_no 54,
  any address in Putnam County Florida.
supabase_county_filter: "county=ilike.%25Putnam%25"
co_no: 54
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 54 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Putnam County Florida zoning ordinance municode",
    "Putnam County Florida GIS zoning layer ArcGIS",
    "Putnam County Florida online zoning map portal",
    "site:municode.com Putnam county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'putnam'})
```
...
- Fallback: FDOR fl_parcels table (co_no=54)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-putnam`
