...
name: county-putnam
description: >
  Zoning intelligence for Putnam County, FL.
  5 jurisdictions, FDOR co_no 54.
  Portal: municode. Anti-scrape: false.
  Triggers: Putnam, putnam county, Palatka, co_no 54.
co_no: 54
county_slug: putnam
...
# Putnam County — Zoning Intelligence

**Seat**: Palatka | **Pop**: 74,521 | **Municipalities**: 5
**FDOR co_no**: 54 | **Portal**: MUNICODE | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Putnam%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.54&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.54&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Putnam County Florida zoning map"`
2. `"Putnam County Florida municode zoning ordinance"`
3. `"Putnam County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/putnam_county
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='putnam'` → Traycer auto-issue.
...
| Municipalities | 5 |
| Portal | MUNICODE |
| Municode | [putnam_county](https://library.municode.com/fl/putnam_county) |
| Anti-scrape | No |
| Rate limit | 20 rpm |
| Phase | P3 |
...
Trigger phrases: "Putnam", "putnam county", "Palatka", "co_no 54"
