---
name: county-santa-rosa
description: >
  Zoning intelligence for Santa Rosa County, FL.
  5 jurisdictions, FDOR co_no 57.
  Portal: arcgis. Anti-scrape: false.
  Triggers: Santa Rosa, santa rosa county, Milton, co_no 57.
co_no: 57
...
# Santa Rosa County — Zoning Intelligence

**Seat**: Milton | **Pop**: 196,077 | **Municipalities**: 5
**FDOR co_no**: 57 | **Portal**: ARCGIS | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Santa Rosa%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.57&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.57&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Santa Rosa County Florida zoning map"`
2. `"Santa Rosa County Florida municode zoning ordinance"`
3. `"Santa Rosa County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/santa_rosa_county
...
**Container**: `modal-county-santa-rosa`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='santa-rosa'` → Traycer auto-issue.
...
| Municipalities | 5 |
| Portal | ARCGIS |
| Municode | [santa_rosa_county](https://library.municode.com/fl/santa_rosa_county) |
| Anti-scrape | No |
| Rate limit | 40 rpm |
| Phase | P3 |
...
Trigger phrases: "Santa Rosa", "santa rosa county", "Milton", "co_no 57"
