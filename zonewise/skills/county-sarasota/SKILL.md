---
name: county-sarasota
description: >
  Zoning intelligence for Sarasota County, FL.
  4 jurisdictions, FDOR co_no 58.
  Portal: arcgis. Anti-scrape: false.
  Triggers: Sarasota, sarasota county, Sarasota, co_no 58.
co_no: 58
...
# Sarasota County — Zoning Intelligence

**Seat**: Sarasota | **Pop**: 434,006 | **Municipalities**: 4
**FDOR co_no**: 58 | **Portal**: ARCGIS | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Sarasota%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.58&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.58&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Sarasota County Florida zoning map"`
2. `"Sarasota County Florida municode zoning ordinance"`
3. `"Sarasota County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/sarasota_county
...
**Container**: `modal-county-sarasota`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='sarasota'` → Traycer auto-issue.
...
| Municipalities | 4 |
| Portal | ARCGIS |
| Municode | [sarasota_county](https://library.municode.com/fl/sarasota_county) |
| Anti-scrape | No |
| Rate limit | 60 rpm |
| Phase | P3 |
...
Trigger phrases: "Sarasota", "sarasota county", "Sarasota", "co_no 58"
