...
description: >
  Zoning intelligence for Seminole County, FL (FDOR co_no: 59).
  ...
  overlay districts. Triggers on: Seminole County, co_no 59,
  any address in Seminole County Florida.
supabase_county_filter: "county=ilike.%25Seminole%25"
co_no: 59
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 59 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Seminole County Florida zoning ordinance municode",
    "Seminole County Florida GIS zoning layer ArcGIS",
    "Seminole County Florida online zoning map portal",
    "site:municode.com Seminole county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'seminole'})
```
...
- Fallback: FDOR fl_parcels table (co_no=59)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-seminole`
