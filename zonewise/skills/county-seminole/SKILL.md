---
name: county-seminole
description: >
  Zoning intelligence for Seminole County, FL.
  7 jurisdictions, FDOR co_no 59.
  Portal: arcgis. Anti-scrape: false.
  Triggers: Seminole, seminole county, Sanford, co_no 59.
co_no: 59
...
# Seminole County — Zoning Intelligence

**Seat**: Sanford | **Pop**: 471,826 | **Municipalities**: 7
**FDOR co_no**: 59 | **Portal**: ARCGIS | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Seminole%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.59&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.59&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Seminole County Florida zoning map"`
2. `"Seminole County Florida municode zoning ordinance"`
3. `"Seminole County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/seminole_county
...
**Container**: `modal-county-seminole`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='seminole'` → Traycer auto-issue.
...
| Municipalities | 7 |
| Portal | ARCGIS |
| Municode | [seminole_county](https://library.municode.com/fl/seminole_county) |
| Anti-scrape | No |
| Rate limit | 60 rpm |
| Phase | P3 |
...
Trigger phrases: "Seminole", "seminole county", "Sanford", "co_no 59"
