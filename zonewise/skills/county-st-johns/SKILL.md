...
description: >
  Zoning intelligence for St. Johns County, FL (FDOR co_no: 55).
  ...
  overlay districts. Triggers on: St. Johns County, co_no 55,
  any address in St. Johns County Florida.
supabase_county_filter: "county=ilike.%25St. Johns%25"
co_no: 55
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 55 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "St. Johns County Florida zoning ordinance municode",
    "St. Johns County Florida GIS zoning layer ArcGIS",
    "St. Johns County Florida online zoning map portal",
    "site:municode.com St. Johns county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'st-johns'})
```
...
- Fallback: FDOR fl_parcels table (co_no=55)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-st-johns`
