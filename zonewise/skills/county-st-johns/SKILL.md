---
name: county-st-johns
description: >
  Zoning intelligence for St. Johns County, FL.
  4 jurisdictions, FDOR co_no 55.
  Portal: arcgis. Anti-scrape: false.
  Triggers: St. Johns, st. johns county, St. Augustine, co_no 55.
co_no: 55
county_slug: st-johns
portal_type: arcgis
anti_scrape: false
rate_limit_rpm: 60
phase: P3
...
# St. Johns County — Zoning Intelligence

**Seat**: St. Augustine | **Pop**: 307,021 | **Municipalities**: 4
**FDOR co_no**: 55 | **Portal**: ARCGIS | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25St. Johns%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.55&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.55&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"St. Johns County Florida zoning map"`
2. `"St. Johns County Florida municode zoning ordinance"`
3. `"St. Johns County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/st._johns_county
...
**Container**: `modal-county-st-johns`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='st-johns'` → Traycer auto-issue.
...
| Municipalities | 4 |
| Portal | ARCGIS |
| Municode | [st._johns_county](https://library.municode.com/fl/st._johns_county) |
| Anti-scrape | No |
| Rate limit | 60 rpm |
| Phase | P3 |
...
Trigger phrases: "St. Johns", "st. johns county", "St. Augustine", "co_no 55"
