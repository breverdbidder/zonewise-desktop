---
name: county-st-lucie
description: >
  Zoning intelligence for St. Lucie County, FL.
  3 jurisdictions, FDOR co_no 56.
  Portal: arcgis. Anti-scrape: false.
  Triggers: St. Lucie, st. lucie county, Fort Pierce, co_no 56.
co_no: 56
county_slug: st-lucie
portal_type: arcgis
anti_scrape: false
rate_limit_rpm: 40
phase: P3
...
# St. Lucie County — Zoning Intelligence

**Seat**: Fort Pierce | **Pop**: 329,226 | **Municipalities**: 3
**FDOR co_no**: 56 | **Portal**: ARCGIS | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25St. Lucie%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.56&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.56&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"St. Lucie County Florida zoning map"`
2. `"St. Lucie County Florida municode zoning ordinance"`
3. `"St. Lucie County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/st._lucie_county
...
**Container**: `modal-county-st-lucie`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='st-lucie'` → Traycer auto-issue.
...
| Municipalities | 3 |
| Portal | ARCGIS |
| Municode | [st._lucie_county](https://library.municode.com/fl/st._lucie_county) |
| Anti-scrape | No |
| Rate limit | 40 rpm |
| Phase | P3 |
...
Trigger phrases: "St. Lucie", "st. lucie county", "Fort Pierce", "co_no 56"
