...
name: county-sumter
description: >
  Zoning intelligence for Sumter County, FL (FDOR co_no: 60).
  ...
  overlay districts. Triggers on: Sumter County, co_no 60,
  any address in Sumter County Florida.
supabase_county_filter: "county=ilike.%25Sumter%25"
co_no: 60
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 60 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Sumter County Florida zoning ordinance municode",
    "Sumter County Florida GIS zoning layer ArcGIS",
    "Sumter County Florida online zoning map portal",
    "site:municode.com Sumter county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'sumter'})
```
...
- Fallback: FDOR fl_parcels table (co_no=60)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-sumter`
