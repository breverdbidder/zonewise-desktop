...
name: county-sumter
description: >
  Zoning intelligence for Sumter County, FL.
  5 jurisdictions, FDOR co_no 60.
  Portal: arcgis. Anti-scrape: false.
  Triggers: Sumter, sumter county, Bushnell, co_no 60.
co_no: 60
county_slug: sumter
portal_type: arcgis
anti_scrape: false
...
# Sumter County — Zoning Intelligence

**Seat**: Bushnell | **Pop**: 132,420 | **Municipalities**: 5
**FDOR co_no**: 60 | **Portal**: ARCGIS | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Sumter%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.60&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.60&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Sumter County Florida zoning map"`
2. `"Sumter County Florida municode zoning ordinance"`
3. `"Sumter County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/sumter_county
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='sumter'` → Traycer auto-issue.
...
| Municipalities | 5 |
| Portal | ARCGIS |
| Municode | [sumter_county](https://library.municode.com/fl/sumter_county) |
| Anti-scrape | No |
| Rate limit | 40 rpm |
| Phase | P3 |
...
Trigger phrases: "Sumter", "sumter county", "Bushnell", "co_no 60"
