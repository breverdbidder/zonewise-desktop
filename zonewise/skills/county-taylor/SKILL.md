...
name: county-taylor
description: >
  Zoning intelligence for Taylor County, FL (FDOR co_no: 62).
  ...
  overlay districts. Triggers on: Taylor County, co_no 62,
  any address in Taylor County Florida.
supabase_county_filter: "county=ilike.%25Taylor%25"
co_no: 62
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 62 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Taylor County Florida zoning ordinance municode",
    "Taylor County Florida GIS zoning layer ArcGIS",
    "Taylor County Florida online zoning map portal",
    "site:municode.com Taylor county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'taylor'})
```
...
- Fallback: FDOR fl_parcels table (co_no=62)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-taylor`
