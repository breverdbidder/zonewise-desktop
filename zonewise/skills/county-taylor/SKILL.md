...
name: county-taylor
description: >
  Zoning intelligence for Taylor County, FL.
  3 jurisdictions, FDOR co_no 62.
  Portal: municode. Anti-scrape: false.
  Triggers: Taylor, taylor county, Perry, co_no 62.
co_no: 62
county_slug: taylor
...
# Taylor County — Zoning Intelligence

**Seat**: Perry | **Pop**: 22,294 | **Municipalities**: 3
**FDOR co_no**: 62 | **Portal**: MUNICODE | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Taylor%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.62&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.62&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Taylor County Florida zoning map"`
2. `"Taylor County Florida municode zoning ordinance"`
3. `"Taylor County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/taylor_county
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='taylor'` → Traycer auto-issue.
...
| Municipalities | 3 |
| Portal | MUNICODE |
| Municode | [taylor_county](https://library.municode.com/fl/taylor_county) |
| Anti-scrape | No |
| Rate limit | 20 rpm |
| Phase | P3 |
...
Trigger phrases: "Taylor", "taylor county", "Perry", "co_no 62"
