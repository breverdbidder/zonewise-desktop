...
name: county-union
description: >
  Zoning intelligence for Union County, FL.
  2 jurisdictions, FDOR co_no 63.
  Portal: pdf. Anti-scrape: false.
  Triggers: Union, union county, Lake Butler, co_no 63.
co_no: 63
county_slug: union
portal_type: pdf
anti_scrape: false
...
# Union County — Zoning Intelligence

**Seat**: Lake Butler | **Pop**: 15,237 | **Municipalities**: 2
**FDOR co_no**: 63 | **Portal**: PDF | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Union%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.63&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.63&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Union County Florida zoning map"`
2. `"Union County Florida municode zoning ordinance"`
3. `"Union County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/union_county
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='union'` → Traycer auto-issue.
...
| Municipalities | 2 |
| Portal | PDF |
| Municode | [union_county](https://library.municode.com/fl/union_county) |
| Anti-scrape | No |
| Rate limit | 10 rpm |
| Phase | P3 |
...
Trigger phrases: "Union", "union county", "Lake Butler", "co_no 63"
