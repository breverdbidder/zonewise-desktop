...
name: county-union
description: >
  Zoning intelligence for Union County, FL (FDOR co_no: 63).
  ...
  overlay districts. Triggers on: Union County, co_no 63,
  any address in Union County Florida.
supabase_county_filter: "county=ilike.%25Union%25"
co_no: 63
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 63 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Union County Florida zoning ordinance municode",
    "Union County Florida GIS zoning layer ArcGIS",
    "Union County Florida online zoning map portal",
    "site:municode.com Union county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'union'})
```
...
- Fallback: FDOR fl_parcels table (co_no=63)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-union`
