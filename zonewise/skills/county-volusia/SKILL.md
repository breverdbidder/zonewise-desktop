...
name: county-volusia
description: >
  Zoning intelligence for Volusia County, FL (FDOR co_no: 64).
  ...
  overlay districts. Triggers on: Volusia County, co_no 64,
  any address in Volusia County Florida.
supabase_county_filter: "county=ilike.%25Volusia%25"
co_no: 64
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 64 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Volusia County Florida zoning ordinance municode",
    "Volusia County Florida GIS zoning layer ArcGIS",
    "Volusia County Florida online zoning map portal",
    "site:municode.com Volusia county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'volusia'})
```
...
- Fallback: FDOR fl_parcels table (co_no=64)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-volusia`
