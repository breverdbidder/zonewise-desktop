...
name: county-wakulla
description: >
  Zoning intelligence for Wakulla County, FL (FDOR co_no: 65).
  ...
  overlay districts. Triggers on: Wakulla County, co_no 65,
  any address in Wakulla County Florida.
supabase_county_filter: "county=ilike.%25Wakulla%25"
co_no: 65
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 65 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Wakulla County Florida zoning ordinance municode",
    "Wakulla County Florida GIS zoning layer ArcGIS",
    "Wakulla County Florida online zoning map portal",
    "site:municode.com Wakulla county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'wakulla'})
```
...
- Fallback: FDOR fl_parcels table (co_no=65)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-wakulla`
