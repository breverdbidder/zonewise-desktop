...
name: county-wakulla
description: >
  Zoning intelligence for Wakulla County, FL.
  1 jurisdictions, FDOR co_no 65.
  Portal: municode. Anti-scrape: false.
  Triggers: Wakulla, wakulla county, Crawfordville, co_no 65.
co_no: 65
county_slug: wakulla
...
# Wakulla County — Zoning Intelligence

**Seat**: Crawfordville | **Pop**: 33,739 | **Municipalities**: 1
**FDOR co_no**: 65 | **Portal**: MUNICODE | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Wakulla%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.65&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.65&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Wakulla County Florida zoning map"`
2. `"Wakulla County Florida municode zoning ordinance"`
3. `"Wakulla County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/wakulla_county
...
**Container**: `modal-county-wakulla`
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='wakulla'` → Traycer auto-issue.
...
| Municipalities | 1 |
| Portal | MUNICODE |
| Municode | [wakulla_county](https://library.municode.com/fl/wakulla_county) |
| Anti-scrape | No |
| Rate limit | 20 rpm |
| Phase | P3 |
...
Trigger phrases: "Wakulla", "wakulla county", "Crawfordville", "co_no 65"
