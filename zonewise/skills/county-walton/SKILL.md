...
name: county-walton
description: >
  Zoning intelligence for Walton County, FL.
  5 jurisdictions, FDOR co_no 66.
  Portal: arcgis. Anti-scrape: false.
  Triggers: Walton, walton county, DeFuniak Springs, co_no 66.
co_no: 66
county_slug: walton
portal_type: arcgis
anti_scrape: false
...
# Walton County — Zoning Intelligence

**Seat**: DeFuniak Springs | **Pop**: 84,923 | **Municipalities**: 5
**FDOR co_no**: 66 | **Portal**: ARCGIS | **Phase**: P3
...
```
GET /jurisdictions?county=ilike.%25Walton%25&select=id,name,data_completeness,municode_url&order=name.asc
```
...
```
GET /fl_parcels?co_no=eq.66&phy_addr1=ilike.%25{street}%25&select=parcel_id,phy_addr1,phy_city,phy_zipcd,dor_uc,centroid_lat,centroid_lng&limit=5
```

### Parcel lookup by parcel ID
```
GET /fl_parcels?co_no=eq.66&parcel_id=eq.{parcel_id}&select=*
```
...
Queries:
1. `"Walton County Florida zoning map"`
2. `"Walton County Florida municode zoning ordinance"`
3. `"Walton County GIS ArcGIS zoning Florida"`
**Output** → UPDATE `jurisdictions.skill_last_validated`

### Mode 2 — Extraction (WebFetch ≤90s)
**Target**: https://library.municode.com/fl/walton_county
...
## Circuit Breaker
All 3 modes fail → INSERT `insights` table with `type='ESCALATE'`, `county='walton'` → Traycer auto-issue.
...
| Municipalities | 5 |
| Portal | ARCGIS |
| Municode | [walton_county](https://library.municode.com/fl/walton_county) |
| Anti-scrape | No |
| Rate limit | 40 rpm |
| Phase | P3 |
...
Trigger phrases: "Walton", "walton county", "DeFuniak Springs", "co_no 66"
