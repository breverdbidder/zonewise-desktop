...
name: county-walton
description: >
  Zoning intelligence for Walton County, FL (FDOR co_no: 66).
  ...
  overlay districts. Triggers on: Walton County, co_no 66,
  any address in Walton County Florida.
supabase_county_filter: "county=ilike.%25Walton%25"
co_no: 66
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 66 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Walton County Florida zoning ordinance municode",
    "Walton County Florida GIS zoning layer ArcGIS",
    "Walton County Florida online zoning map portal",
    "site:municode.com Walton county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'walton'})
```
...
- Fallback: FDOR fl_parcels table (co_no=66)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-walton`
