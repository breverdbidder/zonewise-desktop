...
description: >
  Zoning intelligence for Washington County, FL (FDOR co_no: 67).
  ...
  overlay districts. Triggers on: Washington County, co_no 67,
  any address in Washington County Florida.
supabase_county_filter: "county=ilike.%25Washington%25"
co_no: 67
portal_type: unknown
anti_scrape: false
rate_limit_rpm: 30
...
> **co_no**: 67 | **Phase**: P3 | **Anti-scrape**: false | **Rate limit**: 30 rpm
...
```python
queries = [
    "Washington County Florida zoning ordinance municode",
    "Washington County Florida GIS zoning layer ArcGIS",
    "Washington County Florida online zoning map portal",
    "site:municode.com Washington county florida zoning",
]
...
# Circuit breaker: 3 failures → INSERT insights(type='ESCALATE', meta={county:'washington'})
```
...
- Fallback: FDOR fl_parcels table (co_no=67)

## Escalation Conditions

- 3 consecutive failures → `insights` table `type='ESCALATE'` + auto Traycer issue
- `last_validated > 30 days` → Traycer: `[SKILL] Revalidate county-washington`
